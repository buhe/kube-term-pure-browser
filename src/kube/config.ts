...
export class KubeConfig {
    ...
    public loadFromFile(file: string, opts?: Partial<ConfigOptions>): void {
        ...
        this.loadFromString("todo", opts);
        // this.makePathsAbsolute(rootDirectory);
    }

    public async applytoHTTPSOptions(opts: WebSocket.ClientOptions): Promise<void> {
    ...
    public async applyToRequest(opts: any): Promise<void> {
        const cluster = this.getCurrentCluster();
        const user = this.getCurrentUser();
        ...
        if (cluster && cluster.tlsServerName) {
            opts.agentOptions = { servername: cluster.tlsServerName } as any
        }
    }
    ...
    private applyHTTPSOptions(opts: WebSocket.ClientOptions): void {
        const cluster = this.getCurrentCluster();
        const user = this.getCurrentUser();
        if (!user) {
            return;
